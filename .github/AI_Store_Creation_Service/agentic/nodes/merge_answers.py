...
def merge_answers_node(state: AIStoreAgentState) -> dict[str, Any]:
    try:
        ...
        merged = merge_clarification_answers(
            ...
            previous_confirmed_context=state.get(
                "confirmed_personalization_context", {}
            ),
            latest_ai_context=state.get("effective_personalization_context", {}),
        )
        update: dict[str, Any] = {
            ...
            "merged_personalization_context": deepcopy(merged["merged_personalization_context"]),
            "confirmed_personalization_context": deepcopy(merged["confirmed_personalization_context"]),
            "answered_target_facts": deepcopy(merged["answered_target_facts"]),
