...
from .agentic import blueprinting, feature_flags, generation, merging, repairing, routing, runner, understanding, validation, clarifying
...
from .serializers import (
    AIAgenticClarificationAnswerSerializer,
    AIAgenticClarificationRequestSerializer,
)
from .agentic.personalization import (
...
def _phase6_fashion_personalization_context():
    return {
        "product_offering": "fashion clothing and shoes",
        "catalog_scope": "curated fashion catalog",
        "target_audience": "adult fashion shoppers",
        "target_market": "United States",
        "customer_problem": "finding coordinated clothing and shoes",
        "unique_value_proposition": "a coordinated fashion collection",
        "price_positioning": "mid-range",
        "brand_personality": "modern and practical",
        "visual_preferences": "modern neutral styling",
        "language_currency": "English / USD",
    }


def _explicit_fashion_personalization_description():
    return (
        "We sell fashion clothing and shoes in a curated fashion catalog for "
        "adult fashion shoppers in the United States who need coordinated outfits "
        "with a coordinated fashion collection, mid-range pricing, a modern and "
        "practical brand personality, modern neutral styling visuals, and English "
        "using USD."
    )


def _phase6_fashion_blueprint(templates=None, *, description=None):
    context = _phase6_fashion_personalization_context()
    return blueprinting.build_store_blueprint(
        normalized_description=(
            description or "Create a modern fashion store for women."
        ),
        description_personalization_facts={},
        clarification_facts=context,
        clarification_history=[],
        effective_personalization_context=context,
        available_theme_templates=templates or ["Modern"],
        personalization_core_complete=True,
...
def _phase4_questions(keys):
    return [
        {
            "question_key": key,
            "question_text": f"Please choose {key.replace('_', ' ')}.",
...
class _AgenticE2EProviderFixture:
    """Deterministic provider-boundary fixture with an inspectable call log."""
    ...
    _OPTIONS_BY_KEY = {
        ...
        "target_market": ["United States", "UAE", "Other"],
        "customer_problem": [
            "Finding gentle skincare",
            "Finding trusted natural products",
            "Other",
        ],
        ...
        "price_positioning": ["Premium", "Affordable", "Other"],
        "brand_personality": [
            "Warm and thoughtful",
            "Bold and playful",
            "Other",
        ...
        "visual_preferences": [
            "Minimalist soft pastel",
            "Modern neutral style",
            "أخرى",
        ],
        "language_currency": ["English / USD", "Arabic / SAR", "Other"],
    }
    ...
    def __init__(self):
        ...
        self.final_draft = {
            "store": {
                "name": "Gentle Soap Studio",
                "description": "Handmade natural soaps for gentle daily skincare.",
            },
            "store_settings": {
                "currency": "USD",
                "language": "en",
            ...
            "theme": {
                ...
                "primary_color": "#7A6A45",
                "secondary_color": "#F4E9D8",
                "font_family": "Inter",
                "logo_url": "",
                "banner_url": "",
            },
            "categories": [{"name": "Natural Soaps"}, {"name": "Body Care"}],
            "products": [
                {
                    "name": "Olive Oil Soap",
                    "description": "Handmade soap for gentle skincare.",
                    "price": 18,
                    "sku": "SOAP-OLIVE-01",
                    "category_name": "Natural Soaps",
                    "stock_quantity": 20,
                    "image_url": "",
                },
                {
                    "name": "Lavender Body Soap",
                    "description": "Natural body care soap with lavender.",
                    "price": 22,
                    "sku": "SOAP-LAVENDER-01",
    ...
    def _record(self, operation, kwargs):
        self.calls.append(
            {"operation": operation, "kwargs": deepcopy(kwargs)}
        )

    def analyze_store_description(self, **kwargs):
        self._record("analyze_store_description", kwargs)
        clarification_facts = kwargs.get("clarification_context", {}).get(
            "clarification_facts", {}
        )
        description = kwargs.get("normalized_description", "").casefold()
        complete_description = all(
        ...
        unresolved = [
            key for key in CORE_PERSONALIZATION_KEYS if key not in clarification_facts
        ]
        sufficient = complete_description or not unresolved
        return self._response(
            {
                "description_language": "en",
                "description_sufficient": sufficient,
                "detected_store_domains": ["beauty"],
                "business_summary": "A personalized natural-soap store.",
                "target_audience": "",
                "product_direction": ["natural soaps"],
                "blocking_missing_information": [] if sufficient else [unresolved[0]],
                "ambiguities": [],
            }
        )

    def generate_clarification_questions(self, **kwargs):
        self._record("generate_clarification_questions", kwargs)
        requested_keys = kwargs["semantic_analysis"]["requested_question_keys"]
        questions = []
        for key in requested_keys:
            options = list(self._OPTIONS_BY_KEY[key])
            questions.append(
                {
                    "question_key": key,
                    "question_text": f"Please confirm {key.replace('_', ' ')}.",
                    "options": options,
                    "other_option": options[-1],
                }
            )
        return self._response({"clarification_questions": questions})

    def generate_agentic_store_draft(self, **kwargs):
        self._record("generate_agentic_store_draft", kwargs)
        return self._response(self.final_draft)

    def generate_store_draft(self, **kwargs):
        self._record("generate_store_draft", kwargs)
        return self._response(self.final_draft)

    def clarify_store_draft(self, **kwargs):
        self._record("clarify_store_draft", kwargs)
        return self._response(self.final_draft)

    def regenerate_store_draft(self, **kwargs):
        self._record("regenerate_store_draft", kwargs)
        return self._response(self.final_draft)

    def regenerate_store_draft_section(self, **kwargs):
        self._record("regenerate_store_draft_section", kwargs)
        target = kwargs["target_section"]
        return self._response({target: self.final_draft[target]})


class AIAgenticPersonalizationFoundationTests(SimpleTestCase):
    def test_core_personalization_keys_are_canonical_and_stable(self):
        expected_keys = (
            "product_offering",
            "catalog_scope",
            "target_audience",
        ...
        self.assertEqual(tuple(CORE_PERSONALIZATION_KEYS), expected_keys)
        self.assertEqual(len(CORE_PERSONALIZATION_KEYS), 10)
        self.assertTrue(validate_personalization_key("product_offering"))
        self.assertFalse(validate_personalization_key("not_a_real_key"))

    def test_only_up_to_five_keys_are_selected_each_round(self):
        selected = select_next_personalization_keys({}, limit=50)

        self.assertLessEqual(len(selected), MAX_CLARIFICATION_QUESTIONS_PER_ROUND)
        self.assertEqual(len(selected), MAX_CLARIFICATION_QUESTIONS_PER_ROUND)

    def test_resolved_keys_are_skipped(self):
        selected = select_next_personalization_keys(
            {"product_offering": "A curated fashion collection"},
            limit=10,
        )

        self.assertNotIn("product_offering", selected)
        self.assertTrue(all(key in CORE_PERSONALIZATION_KEYS for key in selected))

    def test_clarification_facts_override_description_extracted_facts(self):
        merged = merge_personalization_facts(
            {"product_offering": "A fashion marketplace", "target_audience": "women"},
            {"product_offering": "A premium skincare marketplace"},
        )

        self.assertEqual(merged["product_offering"], "A premium skincare marketplace")
        self.assertEqual(merged["target_audience"], "women")

    def test_outputs_are_json_serializable(self):
        facts = merge_personalization_facts(
            {"product_offering": "A local bookstore"},
            {"target_audience": "students"},
        )
        payload = {
            "resolved": list(get_resolved_core_personalization_keys(facts)),
            "missing": get_missing_core_personalization_keys(facts),
            "selected": select_next_personalization_keys(facts, limit=5),
        }

        serialized = json.dumps(payload)

        self.assertIn("target_audience", serialized)
        self.assertIn("product_offering", serialized)

    def test_invalid_objects_and_unrelated_words_do_not_become_user_facts(self):
        merged = merge_personalization_facts(
            {
                "product_offering": object(),
                "brand_personality": ["bold"],
                "visual_preferences": {"style": "minimal"},
            },
            {"target_market": 123},
        )
        self.assertEqual(merged, {})
        with self.assertRaises(TypeError):
            ensure_json_serializable({"invalid": object()})

        extracted = extract_description_personalization_facts(
            "We sell luxury candles with bold packaging."
        )
        self.assertEqual(extracted["product_offering"], "luxury candles")
        self.assertNotIn("price_positioning", extracted)
        self.assertNotIn("brand_personality", extracted)

    def test_incomplete_personalization_messages_are_exposed(self):
        self.assertEqual(
            PERSONALIZATION_INCOMPLETE_ERROR_CODE,
            "personalization_information_incomplete",
        )
        self.assertIn(
            "personalized",
            PERSONALIZATION_INCOMPLETE_USER_MESSAGE.casefold(),
        )


class AIAgenticOtherOptionAndCustomAnswerTests(SimpleTestCase):
    @staticmethod
    def _question(question_key="price_positioning", options=None, other_option="Other"):
        return {
            "question_key": question_key,
            "question_text": "What price level should the store target?",
            "options": options or ["Budget", "Mid-range", "Premium", "Luxury", other_option],
            "other_option": other_option,
        }

    @staticmethod
    def _answer(question_key="price_positioning", selected_option="Premium", custom_answer=None):
        answer = {
            "question_key": question_key,
    ...
    def test_normal_option_succeeds_and_canonicalizes(self):
        result = merging.validate_clarification_answer_submission(
            clarification_questions=[self._question()],
            clarification_answers=[self._answer(selected_option=" premium ")],
        )

        self.assertEqual(result[0]["selected_option"], "Premium")
        self.assertNotIn("custom_answer", result[0])

    def test_other_with_valid_custom_answer_is_accepted(self):
        result = merging.validate_clarification_answer_submission(
            clarification_questions=[self._question()],
            clarification_answers=[self._answer(selected_option="Other", custom_answer="Handmade natural soaps")],
        )

        self.assertEqual(result[0]["selected_option"], "Other")
        self.assertEqual(result[0]["custom_answer"], "Handmade natural soaps")

    def test_arabic_other_with_arabic_text_is_accepted(self):
        question = self._question(options=["اقتصادي", "متوسط", "مميز", "أخرى"], other_option="أخرى")
        result = merging.validate_clarification_answer_submission(
            clarification_questions=[question],
            clarification_answers=[self._answer(question_key="price_positioning", selected_option="أخرى", custom_answer="منتجات يدوية")],
        )

        self.assertEqual(result[0]["selected_option"], "أخرى")
        self.assertEqual(result[0]["custom_answer"], "منتجات يدوية")

    def test_other_without_custom_answer_fails(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[self._answer(selected_option="Other")],
            )

    def test_blank_custom_answer_fails(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[self._answer(selected_option="Other", custom_answer="   ")],
            )

    def test_non_string_custom_answer_fails(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[self._answer(selected_option="Other", custom_answer=123)],
            )

    def test_normal_option_with_custom_answer_fails(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[self._answer(selected_option="Premium", custom_answer="Not allowed")],
            )
    ...
    def test_unknown_option_fails(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[self._answer(selected_option="Ultra")],
            )

    def test_duplicate_answers_fail(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question(), self._question(question_key="product_offering")],
                clarification_answers=[
                    self._answer(question_key="price_positioning", selected_option="Premium"),
                    self._answer(question_key="price_positioning", selected_option="Budget"),
                ],
            )
    ...
    def test_extra_fields_fail(self):
        with self.assertRaises(merging.AIMergeValidationError):
            merging.validate_clarification_answer_submission(
                clarification_questions=[self._question()],
                clarification_answers=[{**self._answer(selected_option="Premium"), "extra": "x"}],
            )

    def test_clarification_facts_use_custom_text_for_other_selection(self):
        ...
        self.assertEqual(result["clarification_facts"], {"price_positioning": "Handmade natural soaps"})
        self.assertEqual(result["canonical_answers"][0]["custom_answer"], "Handmade natural soaps")

    def test_invalid_input_does_not_mutate_state_or_counters(self):
        state = {
        ...
        with self.assertRaises(merging.AIMergeValidationError):
            merging.merge_clarification_answers(
                ...
                clarification_facts=state["clarification_facts"],
                clarification_round_count=state["clarification_round_count"],
            )

        self.assertEqual(state, before)

    def test_serializer_accepts_optional_custom_answer(self):
        serializer = AIAgenticClarificationAnswerSerializer(
            data={
        ...
        self.assertEqual(serializer.validated_data["custom_answer"], "Handmade natural soaps")
...
class AIAgenticUnderstandingDecisionTests(SimpleTestCase):
    ...
    def _clarification_payload(self, questions=None):
        return {
            "clarification_questions": questions
            or _phase4_questions(
                list(CORE_PERSONALIZATION_KEYS[:2])
                + list(CORE_PERSONALIZATION_KEYS[3:6])
            )
        }
    ...
    def _assert_valid_mcq_questions(self, questions):
        ...
        for question in questions:
            self.assertEqual(
                set(question),
                {"question_key", "question_text", "options", "other_option"},
            )
            ...
            self.assertEqual(question["options"][-1], question["other_option"])

    def test_vague_description_routes_to_clarify(self):
        provider = self._provider_for_payload(self._analysis_payload(sufficient=True))
        result, _mock_factory = self._understand_and_decide(
            provider,
            self._state("I want a modern store for everyone."),
        )

        self.assertFalse(result["description_sufficient"])
        self.assertEqual(result["route_decision"], "clarify")
        self.assertIn("product_offering", result["missing_core_personalization_keys"])

    def test_description_with_only_product_offering_routes_to_clarify(self):
        provider = self._provider_for_payload(self._analysis_payload(sufficient=True))
        result, _mock_factory = self._understand_and_decide(
            provider,
            self._state("I sell handmade candles."),
        )

        self.assertFalse(result["description_sufficient"])
        self.assertEqual(result["route_decision"], "clarify")
        self.assertIn("target_audience", result["missing_core_personalization_keys"])
        self.assertEqual(result["description_personalization_facts"]["product_offering"], "handmade candles")

    def test_generic_marketing_words_do_not_complete_personalization(self):
        provider = self._provider_for_payload(self._analysis_payload(sufficient=True))
        result, _mock_factory = self._understand_and_decide(
            provider,
            self._state("I want a beautiful modern professional store with good quality products."),
        )

        self.assertFalse(result["description_sufficient"])
        self.assertEqual(result["route_decision"], "clarify")
        self.assertEqual(result["description_personalization_facts"], {})

    def test_detailed_description_with_all_explicit_facts_can_route_to_blueprint(self):
        provider = self._provider_for_payload(self._analysis_payload(sufficient=True))
        result, _mock_factory = self._understand_and_decide(
            provider,
            self._state(
                "Sell handmade candles in a narrow curated catalog for busy moms in the UAE who need safe gifts for birthdays, with handcrafted quality and same-day delivery, positioned at a premium tier, with a warm and thoughtful brand voice, minimalist soft pastel visuals, and English with USD pricing."
            ),
        )

        self.assertTrue(result["personalization_core_complete"])
        self.assertEqual(result["route_decision"], "blueprint")
        self.assertEqual(result["missing_core_personalization_keys"], [])
        self.assertEqual(result["ambiguous_personalization_keys"], [])

    def test_clarification_facts_override_earlier_description_facts(self):
        question = {
            "question_key": "product_offering",
            "question_text": "What should the store sell?",
            "options": ["Candles", "Soaps", "Other"],
            "other_option": "Other",
        }
        answer = {
            "question_key": "product_offering",
            "selected_option": "Other",
            "custom_answer": "handmade soaps",
        }
        result, _mock_factory = self._run_understand_with_provider(
            self._provider_for_payload(self._analysis_payload(sufficient=True)),
            self._state(
                "I sell handmade candles.",
                clarification_facts={"product_offering": "handmade soaps"},
                clarification_round_count=1,
                clarification_history=[
                    {
                        "round_number": 1,
                        ...
                        "answers": [answer],
                        "resolved_facts": {
                            "product_offering": "handmade soaps"
                        },
                    }
                ],
            ),
        ...
        self.assertEqual(result["effective_personalization_context"]["product_offering"], "handmade soaps")

    def test_missing_and_ambiguous_keys_are_represented_correctly(self):
        provider = self._provider_for_payload(
            self._analysis_payload(
                sufficient=False,
                domains=["candles"],
                product_direction=["candles"],
                blocking=["target_audience"],
                ambiguities=["The target audience is ambiguous."],
            )
        )
        result, _mock_factory = self._understand_and_decide(
            provider,
            self._state("I sell candles for women or men."),
        )

        self.assertIn("target_audience", result["ambiguous_personalization_keys"])
        self.assertIn("catalog_scope", result["missing_core_personalization_keys"])
        self.assertNotEqual(result["effective_personalization_context"], {})
    ...
    def test_decide_is_deterministic(self):
        state = {
            **self._state("I sell handmade candles."),
            ...
            "description_personalization_facts": {"product_offering": "handmade candles"},
            "clarification_facts": {},
            "effective_personalization_context": {"product_offering": "handmade candles"},
            "missing_core_personalization_keys": ["catalog_scope", "target_audience"],
            "ambiguous_personalization_keys": [],
            "personalization_core_complete": False,
            "additional_blocking_missing_information": [],
            "description_sufficient": False,
            "blocking_missing_information": [],
        }

        first = decide_node(state)
        second = decide_node(state)

        self.assertEqual(first, second)
        self.assertEqual(first["route_decision"], "clarify")

    def test_decide_does_not_call_the_provider(self):
        state = {
            **self._state("I sell handmade candles."),
            "understanding_valid": True,
            "description_word_count": 4,
    ...
    def test_clear_english_and_arabic_analysis_preserve_ai_semantics(self):
        cases = (
            (
    ...
    def test_decide_remains_deterministic_and_fails_closed_for_malformed_state(self):
        ...
        malformed_states = (
            {"understanding_valid": True},
            {**state, "missing_core_personalization_keys": None},
            {**state, "clarification_round_count": "0"},
            {**state, "ambiguous_personalization_keys": "target_market"},
        )
        ...
        insufficient_state = {
            ...
            **self._analysis_payload(sufficient=False, blocking=["store_domain"]),
            ...
            "missing_core_personalization_keys": ["product_offering"],
            "ambiguous_personalization_keys": [],
            "additional_blocking_missing_information": [],
        }
    ...
    def test_same_description_routes_only_by_ai_analysis(self):
        description = (
            "Sell handmade candles in a narrow curated catalog for busy moms in "
            "the UAE who need safe gifts for birthdays with handcrafted quality "
            "and same-day delivery, positioned at a premium tier, with a warm and "
            "thoughtful brand voice, minimalist soft pastel visuals, and English "
            "with USD pricing."
        )
    ...
    def test_source_boundaries_for_phase_1i(self):
        ...
        self.assertIn("build_personalization_understanding", understand_node_source)
...
class AIAgenticGraphSkeletonTests(SimpleTestCase):
    ...
    def _base_state(
        ...
        **overrides,
        ...
        description = description or _explicit_fashion_personalization_description()
        state: AIStoreAgentState = {
            "workflow_entry": "fresh",
            "store_id": 10,
            "tenant_id": 101,
    ...
    def test_graph_topology_remains_unchanged_for_phase_3(self):
        self.assertEqual(
            self._compiled_graph_edges(),
            {
                (START, "understand", True),
    ...
    def test_clarification_path_reaches_needs_clarification(self):
        result, fake_provider = self._invoke_with_fake_provider(
            ...
            clarification_payload={
                "clarification_questions": _phase4_questions(
                    list(CORE_PERSONALIZATION_KEYS[:5])
                )
            },
        )
...
class AIAgenticGenerateIntegrationTests(SimpleTestCase):
    ...
    def _base_state(self, **overrides):
        effective_context = _phase6_fashion_personalization_context()
        state = {
            "workflow_entry": "fresh",
            "store_id": 10,
            "tenant_id": 101,
            ...
            "clarification_round_count": 0,
            "clarification_history": [],
            "clarification_facts": effective_context,
            "repair_attempt_count": 1,
            "validation_errors": [],
            "effective_personalization_context": effective_context,
        }
        state.update(overrides)
        templates = state.get("available_theme_templates")
        if (
            "blueprint" not in overrides
            and isinstance(templates, list)
            and templates
            and all(isinstance(item, str) and item.strip() for item in templates)
            and isinstance(state.get("normalized_description"), str)
            and state["normalized_description"].strip()
        ):
            state["blueprint"] = _phase6_fashion_blueprint(
                list(dict.fromkeys(" ".join(item.strip().split()) for item in templates)),
    ...
    def _run_graph_with_provider(self, fake_provider, state=None):
        graph_state = deepcopy(state or self._base_state())
        description = _explicit_fashion_personalization_description()
        graph_state.update(
            {
                "user_store_description": description,
                "normalized_description": description,
        ...
        with patch(
            ...
            return compile_agentic_graph().invoke(graph_state)
    ...
    def test_provider_factory_failure_is_logged_without_sensitive_values(self):
        sensitive_values = (
            "PROMPT_SECRET_419",
            "RAW_PROVIDER_RESPONSE_SECRET_419",
            "TOKEN_SECRET_419",
            "CREDENTIAL_SECRET_419",
            "USER_DESCRIPTION_SECRET_419",
            "CLARIFICATION_ANSWER_SECRET_419",
        )
        state = self._base_state(
            store_id=419,
            tenant_id=204,
            user_store_description=sensitive_values[4],
            clarification_answers=[sensitive_values[5]],
        )
        failure = RuntimeError(" | ".join(sensitive_values[:4]))

        with self.assertLogs(
            "AI_Store_Creation_Service.agentic",
            level="ERROR",
        ) as captured, patch(
            "AI_Store_Creation_Service.agentic.generation.get_ai_provider_client",
            side_effect=failure,
        ...
        self._assert_safe_generate_failure(result)
        self.assertIn("store_id=419", logs)
        self.assertIn("tenant_id=204", logs)
        self.assertIn("step=generate", logs)
        self.assertIn("category=provider_factory", logs)
        self.assertIn("exception=RuntimeError", logs)
        for sensitive_value in sensitive_values:
            self.assertNotIn(sensitive_value, logs)

    def test_provider_invocation_and_timeout_failures_have_safe_categories(self):
        cases = (
            (RuntimeError("provider invocation secret"), "provider_invocation"),
            (TimeoutError("provider timeout secret"), "provider_timeout"),
        )

        for exception, expected_category in cases:
            with self.subTest(expected_category=expected_category):
                fake_provider = self.FakeProvider(exception=exception)
                with self.assertLogs(
                    "AI_Store_Creation_Service.agentic",
                    level="ERROR",
                ) as captured:
                ...
                logs = "\n".join(captured.output)
                self._assert_safe_generate_failure(result)
                self.assertIn(f"category={expected_category}", logs)
                self.assertIn(f"exception={type(exception).__name__}", logs)
                self.assertNotIn(str(exception), logs)

    def test_terminal_parse_and_response_schema_failures_are_logged(self):
        parse_provider = self.FakeProvider(
            [
                {"choices": [{"message": {"content": "RAW_SECRET_ONE"}}]},
                {"choices": [{"message": {"content": "RAW_SECRET_TWO"}}]},
            ]
        )
        with self.assertLogs(
            "AI_Store_Creation_Service.agentic",
            ...
            parse_result, _mock_factory = self._run_generate_with_provider(
                parse_provider
            )

        rendered_parse_logs = "\n".join(parse_logs.output)
        self._assert_safe_generate_failure(parse_result)
        self.assertEqual(parse_provider.generate_call_count, 2)
        self.assertIn("category=response_parse", rendered_parse_logs)
        self.assertIn("exception=AIProviderParsingError", rendered_parse_logs)
        self.assertNotIn("RAW_SECRET_ONE", rendered_parse_logs)
        self.assertNotIn("RAW_SECRET_TWO", rendered_parse_logs)

        schema_provider = self.FakeProvider(
            [self._as_provider_response(self._clarification_payload())]
        )
        with self.assertLogs(
            "AI_Store_Creation_Service.agentic",
            ...
            schema_result, _mock_factory = self._run_generate_with_provider(
                schema_provider
            )

        rendered_schema_logs = "\n".join(schema_logs.output)
        self._assert_safe_generate_failure(schema_result)
        self.assertIn("category=response_schema", rendered_schema_logs)
        self.assertIn("exception=ValueError", rendered_schema_logs)

    def test_generate_node_pre_adapter_failure_logs_internal_error(self):
        with self.assertLogs(
            "AI_Store_Creation_Service.agentic.nodes.generate",
            level="ERROR",
        ) as captured, patch(
            "AI_Store_Creation_Service.agentic.generation.get_ai_provider_client"
        ) as mock_factory:
            result = generate_node(self._base_state(user_id=0))

        mock_factory.assert_not_called()
        self._assert_safe_generate_failure(result)
        logs = "\n".join(captured.output)
        self.assertIn("category=internal_error", logs)
        self.assertIn("exception=ValueError", logs)
...
class AIAgenticValidateIntegrationTests(SimpleTestCase):
    ...
    def _state(self, payload=None, mode="draft_ready", **overrides):
        ...
        state = {
            ...
            "effective_personalization_context": effective_context,
            "blueprint": _phase6_fashion_blueprint(),
        }
        state.update(overrides)
        templates = state.get("available_theme_templates")
        if (
    ...
    def _graph_state(self, **overrides):
        description = _explicit_fashion_personalization_description()
        graph_overrides = {
            "user_store_description": description,
            "normalized_description": description,
        ...
        graph_overrides.update(overrides)
        return self._state(**graph_overrides)
    ...
    def test_valid_graph_paths_include_empty_validation_errors(self):
        ...
        with patch(
            ...
            ready = compile_agentic_graph().invoke(self._graph_state())
        with patch(
            "AI_Store_Creation_Service.agentic.understanding.get_ai_provider_client",
            ...
            needs = compile_agentic_graph().invoke(
                self._graph_state(
                    user_store_description="I want an online store.",
                    normalized_description="I want an online store.",
                )
    ...
    def test_invalid_graph_path_enters_repair_then_fails_at_limit(self):
        ...
        with patch(
            ...
            result = compile_agentic_graph().invoke(self._graph_state())
...
class AIAgenticRunnerTests(SimpleTestCase):
    class FakeProvider:
        def __init__(self, payload, analysis_payload=None, clarification_payload=None):
            ...
            self.has_explicit_clarification_payload = clarification_payload is not None
            self.clarification_payload = (
                clarification_payload
        ...
        def generate_clarification_questions(self, **kwargs):
            self.clarification_call_count += 1
            if not self.has_explicit_clarification_payload:
                semantic_analysis = kwargs.get("semantic_analysis", {})
                requested_keys = semantic_analysis.get(
                    "requested_question_keys", []
                )
                return AIAgenticRunnerTests._as_provider_response(
                    {"clarification_questions": _phase4_questions(requested_keys)}
                )
            return AIAgenticRunnerTests._as_provider_response(
                self.clarification_payload
            )
    ...
    def test_clear_english_description_requires_missing_personalization(self):
    ...
    def test_clear_arabic_description_requires_missing_personalization(self):
        description = (
            "\u0623\u0631\u064a\u062f \u0645\u062a\u062c\u0631\u0627 "
            "\u0644\u0628\u064a\u0639 \u0627\u0644\u0642\u0647\u0648\u0629 "
            "\u0627\u0644\u0645\u062e\u062a\u0635\u0629"
...
class AIAgenticClarificationResumeTests(SimpleTestCase):
                ...
                "clarification_questions": [
                    {
                        ...
                        "question_text": "How broad should the catalog be?",
                        "options": ["Curated", "Broad", "Other"],
                        "other_option": "Other",
                    }
                ]
            }
    ...
    def _question(
        ...
        normalized_options = list(options or ["Handmade natural soaps", "Fashion", "Other"])
        if normalized_options[-1] != "Other":
            normalized_options.append("Other")
        return {
            "question_key": question_key,
            "question_text": question_text,
    ...
    @staticmethod
    def _answer(question_key="product_offering", selected_option="Handmade natural soaps"):
        return {
            "question_key": question_key,
            "selected_option": selected_option,
        }
            ...
            "clarification_history": [
                {
                    ...
                    "resolved_facts": {"product_offering": "Handmade natural soaps"},
                }
            ],
            "clarification_facts": {"product_offering": "Handmade natural soaps"},
            "repair_attempt_count": 0,
            "current_step": "human_review",
            "mode": "draft_ready",
            "status": WORKFLOW_STATUS_READY_FOR_REVIEW,
    ...
    def test_resume_becomes_sufficient_and_reaches_ready_for_review(self):
        provider = self.FakeResumeProvider(
            ...
            draft_payload=_AgenticE2EProviderFixture().final_draft,
        )
        description = (
            "We sell placeholder products in a narrow curated catalog for busy moms "
            "in UAE who need gentle skincare "
            "with handcrafted same-day delivery, premium pricing, a warm thoughtful "
            "brand, minimalist soft pastel visuals, in English using USD."
        )
        ...
        result = self._run_resume_with_provider(
            ...
            [self._answer(selected_option=" handmade natural soaps ")],
            provider,
        )
        ...
        self.assertEqual(
            result["clarification_facts"],
            {"product_offering": "Handmade natural soaps"},
        )
        self.assertEqual(result["clarification_questions"], [])
        self.assertEqual(provider.analysis_call_count, 1)
        self.assertEqual(provider.generate_call_count, 1)
    ...
    def test_resume_still_insufficient_asks_only_unresolved_question(self):
        provider = self.FakeResumeProvider(
            analysis_payload=self._insufficient_analysis_payload("catalog_scope"),
            clarification_payload={
                "clarification_questions": _phase4_questions(
                    list(CORE_PERSONALIZATION_KEYS[1:6])
                )
            },
        )
    ...
    def test_resolved_key_cannot_remain_blocking_after_resume(self):
        provider = self.FakeResumeProvider(
            ...
            clarification_payload={
                "clarification_questions": [self._question("product_offering")]
            },
        )
...
class AIAgenticCachedSessionServiceTests(TestCase):
    ...
    @staticmethod
    def _question(question_key="product_offering"):
        return {
            "question_key": question_key,
            "question_text": "What type of store should be created?",
    ...
    def test_resume_cache_miss_returns_failure_without_graph_call(self):
        with patch(
            ...
            result = resume_cached_agentic_workflow(
                ...
                user_id=7,
                clarification_answers=[
                    {"question_key": "product_offering", "selected_option": "Coffee"}
                ],
            )
    ...
    def test_resume_success_to_ready_overwrites_cached_state(self):
            ...
            clarification_history=[
                {
                    ...
                    "resolved_facts": {"product_offering": "Coffee"},
                }
            ],
            clarification_facts={"product_offering": "Coffee"},
        )
        ...
        self.assertEqual(cached["clarification_facts"], {"product_offering": "Coffee"})
    ...
    def test_invalid_answers_failure_does_not_destroy_prior_session(self):
        ...
        with patch(
            ...
            result = resume_cached_agentic_workflow(
                ...
                tenant_id=101,
                user_id=7,
                clarification_answers=[{"question_key": "product_offering", "selected_option": "Other"}],
            )
...
class AIProviderSelectionTests(TestCase):
    ...
    def test_ollama_generate_clarification_questions_uses_prompt_builder_and_transport(
        ...
        provider.generate_clarification_questions(
            ...
            semantic_analysis={
                ...
                "target_audience": "",
                "product_direction": [],
                "blocking_missing_information": ["product_offering"],
                "ambiguities": ["The store domain is missing."],
                "requested_question_keys": ["product_offering"],
                "requested_question_specs": [
                    {
                        "question_key": "product_offering",
                        "purpose": "The core product or service being sold.",
                        "question_type": "core",
                    }
                ],
        ...
        self.assertIn("backend has already selected the exact question keys", messages[0]["content"])
        self.assertIn("normalized_description: I want an online store", messages[3]["content"])
        self.assertIn('"requested_question_keys": ["product_offering"]', messages[4]["content"])
        self.assertIn("clarification_round_count: 1", messages[7]["content"])
    ...
    def test_clarification_questions_prompt_contract(self):
        ...
        for expected_text in (
            "clarify node",
            "backend has already selected the exact question keys",
            "return exactly one question for every requested key",
            "preserve requested key order",
            "add no extra key",
            "between 1 and 5 questions",
            "between 3 and 5 unique options",
            "the other option must be the final option",
        ...
        self.assertIn("requested_question_keys:", messages[5]["content"])
        self.assertIn("clarification_round_count: 1", messages[7]["content"])
...
class AIAgenticApiEndToEndTests(AICreationApiTests):
    _PUBLIC_RESPONSE_KEYS = {"store_id", "draft_payload", "draft_metadata"}
    _INTERNAL_PUBLIC_KEYS = {
        "tenant_id",
        "user_id",
        "workflow_entry",
        "route_decision",
    ...
    @staticmethod
    def _provider_boundary(provider):
        stack = ExitStack()
        for target in (
            "AI_Store_Creation_Service.services.get_ai_provider_client",
            "AI_Store_Creation_Service.agentic.understanding.get_ai_provider_client",
    ...
    def _start_vague_agentic_flow(self):
        return self.client.post(
            reverse("ai_store_creation:start-draft"),
            {
                "user_description": (
                    "Please help me create a beautiful personalized online retail experience."
                )
            },
            format="json",
        )
        ...
        return [
            {
                ...
                "selected_option": next(
                    ...
                    if option != question["other_option"]
                ),
            }
            for question in questions
        ]

    def _cached_state(self, store_id):
        return get_cached_agentic_workflow(
            store_id=store_id,
            tenant_id=self.user.tenant_id,
            user_id=self.user.id,
        )

    def _assert_public_agentic_response(self, body):
        self.assertEqual(set(body), self._PUBLIC_RESPONSE_KEYS)
        self.assertEqual(body["draft_metadata"]["workflow_engine"], "agentic")
        ...
        def visit(value):
            if isinstance(value, dict):
                for key, nested in value.items():
                    self.assertNotIn(key, self._INTERNAL_PUBLIC_KEYS)
                    self.assertNotIn("traceback", key.casefold())
                    self.assertNotIn("cache", key.casefold())
                    visit(nested)
    ...
    def _assert_progress(self, body, *, resolved, missing):
        progress = body["draft_metadata"]["personalization_progress"]
        self.assertEqual(
            progress,
            {
                "resolved_core_count": resolved,
                "total_core_count": 10,
                "core_complete": resolved == 10,
                "missing_core_keys": list(missing),
            },
        )

    def _assert_question_contract(self, questions):
        ...
        for question in questions:
            ...
            self.assertEqual(options.count(question["other_option"]), 1)
            self.assertEqual(options[-1], question["other_option"])
            self.assertIn(question["other_option"], {"Other", "أخرى"})

    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_clear_description_agentic_clarification_flow(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            response = self.client.post(
                reverse("ai_store_creation:start-draft"),
                {"user_description": "Create a modern coffee store for young customers."},
                format="json",
            )
        ...
        body = self._payload(response)
        self.assertEqual(set(body.keys()), {"store_id", "draft_payload", "draft_metadata"})
        self.assertTrue(body["draft_payload"]["clarification_needed"])
        self.assertEqual(body["draft_metadata"]["workflow_engine"], "agentic")
        self.assertEqual(body["draft_metadata"]["status"], WORKFLOW_STATUS_NEEDS_CLARIFICATION)

    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_vague_description_and_valid_clarify_advances_progress(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start_response = self.client.post(
                reverse("ai_store_creation:start-draft"),
                {"user_description": "I want to create a beautiful modern online store for customers"},
                format="json",
            )
            self.assertEqual(start_response.status_code, 201)
            start_body = start_response.json()
            store_id = start_body["store_id"]
            questions = start_body["draft_payload"]["clarification_questions"]
        ...
        self.assertEqual(body["store_id"], store_id)
        self.assertEqual(body["draft_metadata"]["status"], WORKFLOW_STATUS_NEEDS_CLARIFICATION)
        self.assertEqual(body["draft_metadata"]["clarification_round_count"], 1)

    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_detailed_description_reaches_ready_with_complete_safe_progress(self):
        provider = _AgenticE2EProviderFixture()
        description = (
            "We sell handmade natural soaps in a curated catalog for women in Dubai "
            "who need gentle skincare with handcrafted same-day delivery, premium "
            "pricing, a warm thoughtful brand, minimalist soft pastel visuals, in "
            "English using USD."
        )
        ...
        body = response.json()
        self._assert_public_agentic_response(body)
        self._assert_progress(body, resolved=10, missing=[])
        self.assertEqual(
            body["draft_metadata"]["status"],
            WORKFLOW_STATUS_READY_FOR_REVIEW,
        )
        self.assertFalse(body["draft_payload"]["clarification_needed"])
        self.assertEqual(body["draft_payload"]["clarification_questions"], [])
        self.assertEqual(body["draft_metadata"]["clarification_round_count"], 0)
        self.assertEqual(body["draft_metadata"]["repair_attempt_count"], 0)
        self.assertEqual(body["draft_metadata"]["max_clarification_rounds"], 3)
        self.assertEqual(body["draft_metadata"]["max_repair_attempts"], 3)
        self.assertEqual(
            [call["operation"] for call in provider.calls],
            ["analyze_store_description", "generate_agentic_store_draft"],
        )

        current = self.client.get(
            reverse(
                "ai_store_creation:current-draft",
    ...
    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_two_round_flow_preserves_question_contract_custom_answers_and_progress(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start_response = self._start_vague_agentic_flow()
            self.assertEqual(start_response.status_code, 201, start_response.content)
            start_body = start_response.json()
            self._assert_public_agentic_response(start_body)
            self._assert_progress(
                start_body,
                resolved=0,
                missing=CORE_PERSONALIZATION_KEYS,
            )
            first_questions = start_body["draft_payload"]["clarification_questions"]
            self._assert_question_contract(first_questions)
            self.assertEqual(
                [question["question_key"] for question in first_questions],
                list(CORE_PERSONALIZATION_KEYS[:5]),
            )

            first_response = self.client.post(
                reverse(
                    "ai_store_creation:clarify-draft",
                    kwargs={"store_id": start_body["store_id"]},
                ),
                {"clarification_answers": self._normal_answers(first_questions)},
                format="json",
            )
            self.assertEqual(first_response.status_code, 200, first_response.content)
            first_body = first_response.json()
            self._assert_public_agentic_response(first_body)
            self._assert_progress(
                first_body,
                resolved=5,
                missing=CORE_PERSONALIZATION_KEYS[5:],
            )
            second_questions = first_body["draft_payload"]["clarification_questions"]
            self._assert_question_contract(second_questions)
            first_keys = {question["question_key"] for question in first_questions}
            second_keys = {question["question_key"] for question in second_questions}
            self.assertFalse(first_keys.intersection(second_keys))
            self.assertEqual(
                [question["question_key"] for question in second_questions],
                list(CORE_PERSONALIZATION_KEYS[5:]),
            )

            second_answers = self._normal_answers(second_questions)
            for answer, question in zip(second_answers, second_questions):
                if question["question_key"] == "brand_personality":
                    answer["selected_option"] = question["other_option"]
                    answer["custom_answer"] = "Calm and trustworthy"
                elif question["question_key"] == "visual_preferences":
                    self.assertEqual(question["other_option"], "أخرى")
                    answer["selected_option"] = question["other_option"]
                    answer["custom_answer"] = "ألوان ترابية هادئة"
            ...
            final_response = self.client.post(
                ...
                {"clarification_answers": second_answers},
                format="json",
            )

        self.assertEqual(final_response.status_code, 200, final_response.content)
        final_body = final_response.json()
        self._assert_public_agentic_response(final_body)
        self._assert_progress(final_body, resolved=10, missing=[])
        self.assertEqual(
            final_body["draft_metadata"]["status"],
            WORKFLOW_STATUS_READY_FOR_REVIEW,
        )
        self.assertEqual(final_body["draft_metadata"]["clarification_round_count"], 2)
        self.assertEqual(final_body["draft_metadata"]["repair_attempt_count"], 0)
        self.assertNotEqual(
            final_body["draft_metadata"]["status"],
            WORKFLOW_STATUS_APPLIED,
        )

        cached = self._cached_state(start_body["store_id"])
        self.assertIsNotNone(cached)
        self.assertEqual(
            cached["clarification_facts"]["brand_personality"],
            "Calm and trustworthy",
        )
        self.assertEqual(
            cached["clarification_facts"]["visual_preferences"],
            "ألوان ترابية هادئة",
        )
        self.assertEqual(len(cached["clarification_history"]), 2)
        self.assertEqual(
            [call["operation"] for call in provider.calls],
            [
    ...
    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_invalid_batches_leave_cached_session_and_counters_unchanged(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start_response = self._start_vague_agentic_flow()
            self.assertEqual(start_response.status_code, 201)
            ...
            store_id = start_body["store_id"]
            questions = start_body["draft_payload"]["clarification_questions"]
            normal_answers = self._normal_answers(questions)

            invalid_batches = []

            missing_custom = deepcopy(normal_answers)
            missing_custom[0]["selected_option"] = questions[0]["other_option"]
            invalid_batches.append(missing_custom)

            custom_with_normal = deepcopy(normal_answers)
            custom_with_normal[0]["custom_answer"] = "Should be rejected"
            invalid_batches.append(custom_with_normal)

            unknown_option = deepcopy(normal_answers)
            unknown_option[0]["selected_option"] = "Unknown provider option"
            invalid_batches.append(unknown_option)

            one_character_custom = deepcopy(normal_answers)
            one_character_custom[0]["selected_option"] = questions[0]["other_option"]
            one_character_custom[0]["custom_answer"] = "x"
            invalid_batches.append(one_character_custom)

            for answers in invalid_batches:
                with self.subTest(answers=answers):
                    before = self._cached_state(store_id)
                    before_json = json.dumps(before, ensure_ascii=False, sort_keys=True)
                    provider_calls_before = deepcopy(provider.calls)
                    ...
                    after = self._cached_state(store_id)
                    self.assertEqual(
                        json.dumps(after, ensure_ascii=False, sort_keys=True),
                        before_json,
                    )
                    self.assertEqual(
                        after["clarification_round_count"],
                        before["clarification_round_count"],
                    )
                    self.assertEqual(
                        after["repair_attempt_count"],
                        before["repair_attempt_count"],
                    )
                    self.assertEqual(after["clarification_facts"], before["clarification_facts"])
                    self.assertEqual(
                        after["clarification_history"],
                        before["clarification_history"],
                    )
                    self.assertEqual(provider.calls, provider_calls_before)
            ...
            successful = self.client.post(
                ...
                {"clarification_answers": normal_answers},
                format="json",
            )

        self.assertEqual(successful.status_code, 200, successful.content)
        resumed = self._cached_state(store_id)
        self.assertEqual(resumed["clarification_round_count"], 1)
        self.assertEqual(resumed["repair_attempt_count"], 0)
        self.assertEqual(len(resumed["clarification_history"]), 1)
        self.assertTrue(resumed["clarification_facts"])
        self.assertEqual(
            [call["operation"] for call in provider.calls],
            [
                "analyze_store_description",
    ...
    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_jwt_owner_and_tenant_isolation_use_the_real_api_stack(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start = self._start_vague_agentic_flow()
        self.assertEqual(start.status_code, 201)
        store_id = start.json()["store_id"]
        current_url = reverse(
            "ai_store_creation:current-draft",
            kwargs={"store_id": store_id},
        )
        ...
        self.client.credentials()
        unauthorized = self.client.get(current_url)
        self.assertEqual(unauthorized.status_code, 401)

        self._authenticate(self.other_owner_same_tenant)
        wrong_owner = self.client.get(current_url)
        ...
        other_tenant_owner.tenant_id = 202
        other_tenant_owner.save(update_fields=["is_active", "tenant_id"])
        self._authenticate(other_tenant_owner)
        wrong_tenant = self.client.get(current_url)
        self.assertEqual(wrong_tenant.status_code, 404)

        self._authenticate(self.user)
        authorized = self.client.get(current_url)
        self.assertEqual(authorized.status_code, 200)
        self._assert_public_agentic_response(authorized.json())

    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_existing_session_keeps_agentic_affinity_when_flag_is_disabled(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start = self._start_vague_agentic_flow()
            self.assertEqual(start.status_code, 201)
            ...
            with override_settings(AI_AGENTIC_WORKFLOW_ENABLED=False):
                resumed = self.client.post(
                    reverse(
                        "ai_store_creation:clarify-draft",
                        kwargs={"store_id": store_id},
                    ),
                    {"clarification_answers": self._normal_answers(questions)},
                    format="json",
                )
                ...
                legacy_start = self.client.post(
                    ...
                    {
                        "user_description": (
                            "Create a complete natural soap store for local customers."
                        )
                    },
                    format="json",
                )
        ...
        self._assert_public_agentic_response(resumed_body)
        self.assertEqual(resumed_body["draft_metadata"]["clarification_round_count"], 1)
        self.assertIsNotNone(self._cached_state(store_id))
        self.assertIsNone(get_ai_draft(store_id))
        self.assertIsNone(get_ai_draft_meta(store_id))

        self.assertEqual(legacy_start.status_code, 201, legacy_start.content)
        legacy_body = legacy_start.json()
        self.assertEqual(set(legacy_body), self._PUBLIC_RESPONSE_KEYS)
        self.assertNotEqual(
            legacy_body["draft_metadata"].get("workflow_engine"),
            "agentic",
        )
        self.assertIsNone(
            get_cached_agentic_workflow(
                store_id=legacy_body["store_id"],
                tenant_id=self.user.tenant_id,
                user_id=self.user.id,
            )
        )
    ...
    @override_settings(AI_AGENTIC_WORKFLOW_ENABLED=True)
    def test_agentic_regeneration_and_apply_are_rejected_without_applied_status(self):
        provider = _AgenticE2EProviderFixture()
        with self._provider_boundary(provider):
            start = self._start_vague_agentic_flow()
            self.assertEqual(start.status_code, 201)
            ...
            responses = (
                self.client.post(
                    reverse(
                        "ai_store_creation:regenerate-draft",
                        kwargs={"store_id": store_id},
                    ),
                    {},
                    format="json",
        ...
        for response in responses:
            with self.subTest(path=response.wsgi_request.path):
                self.assertEqual(response.status_code, 400, response.content)
                self.assertNotIn("applied", json.dumps(response.json()).casefold())
        ...
        self.assertEqual(current.status_code, 200)
        self._assert_public_agentic_response(current.json())
        self.assertNotEqual(
            current.json()["draft_metadata"]["status"],
            WORKFLOW_STATUS_APPLIED,
        )



class AIAgenticDeterministicClarificationBatchingTests(SimpleTestCase):
    @staticmethod
    def _question(question_key, *, other_option="Other"):
        return {
            "question_key": question_key,
            "question_text": f"Please confirm {question_key.replace('_', ' ')}.",
            "options": ["Option A", "Option B", other_option],
            "other_option": other_option,
        }

    @staticmethod
    def _all_core_facts():
        return {key: f"confirmed {key}" for key in CORE_PERSONALIZATION_KEYS}

    def _select(
        self,
        *,
        description_facts=None,
        clarification_facts=None,
        missing=None,
        ambiguous=None,
        blocking=None,
        round_count=0,
        ...
        return select_clarification_question_keys(
            description_personalization_facts=description_facts or {},
            clarification_facts=clarification_facts or {},
            missing_core_personalization_keys=(
                list(CORE_PERSONALIZATION_KEYS) if missing is None else missing
            ),
            ambiguous_personalization_keys=ambiguous or [],
            additional_blocking_missing_information=blocking or [],
            clarification_round_count=round_count,
            clarification_history=history or [],
        )

    def test_round_one_and_round_two_use_canonical_batches(self):
        first_batch = self._select(round_count=0)
        self.assertEqual(first_batch, list(CORE_PERSONALIZATION_KEYS[:5]))

        first_facts = {key: f"answer {key}" for key in first_batch}
        history = [
            {
                "questions": [self._question(key) for key in first_batch],
            }
        ]
        second_batch = self._select(
            clarification_facts=first_facts,
            missing=list(CORE_PERSONALIZATION_KEYS[5:]),
            round_count=1,
            history=history,
        )
        self.assertEqual(second_batch, list(CORE_PERSONALIZATION_KEYS[5:]))

    def test_selection_never_exceeds_five_and_skips_known_facts(self):
        selected = self._select(
            description_facts={"product_offering": "handmade soaps"},
            clarification_facts={"catalog_scope": "specialized"},
            round_count=0,
        )
    ...
    def test_previously_asked_keys_are_not_repeated(self):
        history = [
            {
                "questions": [
                    self._question("product_offering"),
                    self._question("catalog_scope"),
                ]
            }
        ]
        selected = self._select(round_count=0, history=history)
        self.assertNotIn("product_offering", selected)
        self.assertNotIn("catalog_scope", selected)

    def test_round_three_is_adaptive_only(self):
        all_facts = self._all_core_facts()
        selected = self._select(
            description_facts=all_facts,
            missing=[],
            blocking=["shipping_model", "primary_color"],
            round_count=2,
        )
        self.assertEqual(selected, ["shipping_model"])

        core_still_missing = self._select(
            description_facts={"product_offering": "coffee"},
            missing=["target_audience"],
            round_count=2,
        )
        self.assertEqual(core_still_missing, [])

    def test_provider_payload_must_match_exact_requested_keys_and_other_contract(self):
        requested = ["product_offering", "catalog_scope"]
        payload = {
            "clarification_questions": [self._question(key) for key in requested]
        }
        result = clarifying.validate_clarification_questions_payload(
            payload,
            requested_question_keys=requested,
            clarification_facts={},
            clarification_history=[],
        )
        self.assertEqual([item["question_key"] for item in result], requested)
        self.assertTrue(all(item["options"][-1] == item["other_option"] for item in result))

    def test_malformed_renamed_extra_duplicate_or_missing_other_fails_closed(self):
        valid = self._question("product_offering")
        cases = (
            {"clarification_questions": [{**valid, "question_key": "renamed_key"}]},
            {
                "clarification_questions": [
                    valid,
                    self._question("catalog_scope"),
            ...
            {
                "clarification_questions": [
                    {key: value for key, value in valid.items() if key != "other_option"}
                ]
            },
            {
                "clarification_questions": [
                    {
                        **valid,
                        "options": ["Option A", "Other", "Option B"],
                    }
                ]
            },
            {
        ...
        for payload in cases:
            with self.subTest(payload=payload):
                with self.assertRaises(clarifying.AIClarificationValidationError):
                    clarifying.validate_clarification_questions_payload(
                        payload,
                        requested_question_keys=["product_offering"],
                        clarification_facts={},
                        clarification_history=[],
    ...
    def test_question_specs_preserve_backend_order(self):
        keys = ["target_market", "customer_problem"]
        specs = build_clarification_question_specs(keys)
        self.assertEqual([spec["question_key"] for spec in specs], keys)
        self.assertTrue(all(spec["question_type"] == "core" for spec in specs))

    def test_clarify_node_sends_exact_selected_keys_to_provider(self):
        class FakeProvider:
            def __init__(self):
                self.calls = []
            ...
            def generate_clarification_questions(self, **kwargs):
                ...
                requested = kwargs["semantic_analysis"]["requested_question_keys"]
                payload = {
                    "clarification_questions": [
                        AIAgenticDeterministicClarificationBatchingTests._question(key)
        ...
        state = {
            ...
            "tenant_id": 101,
            "user_id": 7,
            "normalized_description": "I want a store.",
            "description_language": "en",
            "description_sufficient": False,
            "detected_store_domains": [],
            "business_summary": "",
            ...
            "description_personalization_facts": {},
            "effective_personalization_context": {},
            "missing_core_personalization_keys": list(CORE_PERSONALIZATION_KEYS),
            "ambiguous_personalization_keys": [],
            "additional_blocking_missing_information": ["core_personalization_missing"],
            "clarification_round_count": 0,
            "clarification_history": [],
            "clarification_facts": {},
            "repair_attempt_count": 2,
        ...
        expected = list(CORE_PERSONALIZATION_KEYS[:5])
        self.assertEqual(result["clarification_requested_keys"], expected)
        self.assertEqual(
            provider.calls[0]["semantic_analysis"]["requested_question_keys"],
            expected,
        )
        self.assertEqual(len(result["clarification_questions"]), 5)
        self.assertNotIn("clarification_round_count", result)
        self.assertNotIn("repair_attempt_count", result)

    def test_max_round_incomplete_personalization_fails_safely(self):
        state = {
            "understanding_valid": True,
            "missing_core_personalization_keys": ["language_currency"],
            "ambiguous_personalization_keys": [],
            "additional_blocking_missing_information": [],
            "clarification_round_count": 2,
            "repair_attempt_count": 1,
        ...
        result = decide_node(state)
        self.assertEqual(result["route_decision"], "failed_recoverable")
        self.assertEqual(result["error_code"], PERSONALIZATION_INCOMPLETE_ERROR_CODE)
        self.assertEqual(result["user_message"], PERSONALIZATION_INCOMPLETE_USER_MESSAGE)
        self.assertNotIn("clarification_round_count", result)
        self.assertNotIn("repair_attempt_count", result)

    def test_clarification_and_repair_counters_remain_independent(self):
        state = {
            "understanding_valid": True,
            "missing_core_personalization_keys": ["target_audience"],
            "ambiguous_personalization_keys": [],
            "additional_blocking_missing_information": [],
            "clarification_round_count": 1,
            "repair_attempt_count": 2,
...
class AIAgenticPersonalizationConstrainedBlueprintGenerationTests(SimpleTestCase):
    """Focused Phase 5 tests for locked Blueprint and generation constraints."""

    @staticmethod
    def _description_facts():
        return {
            "product_offering": "handmade natural soaps",
            "catalog_scope": "specialized narrow soap catalog",
        }

    @staticmethod
    def _clarification_facts():
        return {
            "target_audience": "eco-conscious women with sensitive skin",
            "target_market": "Syria",
            "customer_problem": "finding gentle locally available skin cleansing",
            "unique_value_proposition": "small-batch natural Syrian ingredients",
            "price_positioning": "luxury",
            "brand_personality": "calm, refined, and trustworthy",
            "visual_preferences": "earth tones with warm beige and olive green",
            "language_currency": "Arabic / SYP",
        }

    def _effective_context(self):
        return merge_personalization_facts(
            self._description_facts(),
            self._clarification_facts(),
        )

    def _blueprint_state(self, **overrides):
        description_facts = self._description_facts()
        clarification_facts = self._clarification_facts()
        effective = merge_personalization_facts(
            description_facts,
            clarification_facts,
        )
        state = {
            ...
            "tenant_id": 101,
            "user_id": 7,
            "user_store_description": "متجر متخصص للصابون الطبيعي اليدوي في سوريا",
            "normalized_description": "متجر متخصص للصابون الطبيعي اليدوي في سوريا",
            "available_theme_templates": ["Earth", "Minimal"],
            "understanding_valid": True,
            "description_sufficient": True,
            ...
            "clarification_history": [
                {
                    ...
                    "questions": [],
                    "answers": [],
                    "resolved_facts": deepcopy(clarification_facts),
                }
            ],
    ...
    @staticmethod
    def _valid_arabic_soap_payload():
        return {
            "store": {
                "name": "نقاء الشام",
                "description": "متجر سوري فاخر للصابون الطبيعي المصنوع يدويًا للبشرة الحساسة.",
            },
            "store_settings": {
                "currency": "SYP",
                "language": "ar",
            ...
            "theme": {
                "theme_template": "Earth",
                "primary_color": "#7A6A45",
                "secondary_color": "#D8C9A7",
                "font_family": "Cairo",
                "logo_url": "",
                "banner_url": "",
            },
            ...
            "products": [
                {
                    ...
                    "description": "صابون يدوي لطيف بزيت الزيتون مناسب للعناية اليومية بالبشرة الحساسة.",
                    "price": 85000,
                    "sku": "SOAP-OLIVE-01",
                ...
                {
                    "name": "مجموعة العناية الهادئة",
                    "description": "مجموعة فاخرة من الصابون الطبيعي بروائح هادئة وتغليف أنيق للهدايا.",
                    "price": 240000,
                    "sku": "CARE-SET-01",
                    "category_name": "مجموعات عناية",
                    "stock_quantity": 10,
                    "image_url": "",
                },
            ],
    ...
    def _build_blueprint(self, state=None):
        base_state = state or self._blueprint_state()
        update = blueprint_node(base_state)
        self.assertEqual(update["route_decision"], "generate")
        self.assertIn("blueprint", update)
        return update["blueprint"]

    def test_blueprint_receives_and_represents_all_ten_core_facts(self):
        state = self._blueprint_state()
        before = deepcopy(state)

        update = blueprint_node(state)
        ...
        for key in CORE_PERSONALIZATION_KEYS[:-1]:
            self.assertEqual(
                blueprint[key],
                state["effective_personalization_context"][key],
            )
        self.assertEqual(blueprint["language"], "ar")
        self.assertEqual(blueprint["currency"], "SYP")
        self.assertEqual(
            blueprint["source_context"]["description_personalization_facts"],
            state["description_personalization_facts"],
        )
        self.assertEqual(
            blueprint["source_context"]["clarification_facts"],
            state["clarification_facts"],
        )
        self.assertEqual(
            blueprint["source_context"]["clarification_history"],
            state["clarification_history"],
        )
        self.assertEqual(
            blueprint["available_theme_templates"],
            state["available_theme_templates"],
        )
        json.dumps(blueprint, ensure_ascii=False)

    def test_blueprint_fails_before_generation_when_core_or_ambiguity_is_unresolved(self):
        cases = (
            {"personalization_core_complete": False},
            {"missing_core_personalization_keys": ["target_market"]},
            {"ambiguous_personalization_keys": ["target_audience"]},
            {"additional_blocking_missing_information": ["delivery_model"]},
        )
        for overrides in cases:
            with self.subTest(overrides=overrides):
                result = blueprint_node(self._blueprint_state(**overrides))
                self.assertEqual(result["status"], WORKFLOW_STATUS_FAILED_RECOVERABLE)
                self.assertEqual(result["route_decision"], "failed_recoverable")
                self.assertNotIn("blueprint", result)

    def test_blueprint_validation_failure_is_logged_without_sensitive_context(self):
        sensitive_values = (
            "BLUEPRINT_DESCRIPTION_SECRET",
            "BLUEPRINT_CLARIFICATION_SECRET",
            "BLUEPRINT_TOKEN_SECRET",
        )
        state = self._blueprint_state(
            store_id=419,
            tenant_id=204,
            user_store_description=sensitive_values[0],
            clarification_answers=[sensitive_values[1]],
            provider_token=sensitive_values[2],
            personalization_core_complete=False,
        )

        with self.assertLogs(
            "AI_Store_Creation_Service.agentic.nodes.blueprint",
            level="ERROR",
        ) as captured:
            result = blueprint_node(state)
        ...
        self.assertIn("store_id=419", logs)
        self.assertIn("tenant_id=204", logs)
        self.assertIn("step=blueprint", logs)
        self.assertIn("category=blueprint_validation", logs)
        self.assertIn("exception=ValueError", logs)
        for sensitive_value in sensitive_values:
            self.assertNotIn(sensitive_value, logs)

    def test_generation_receives_blueprint_and_effective_personalization_context(self):
        blueprint = self._build_blueprint()
        calls = []

        class FakeProvider:
            def generate_agentic_store_draft(inner_self, **kwargs):
                calls.append(deepcopy(kwargs))
                return self._raw_response(self._valid_arabic_soap_payload())

        state = {
            **self._blueprint_state(),
            "blueprint": blueprint,
        }
        with patch(
            "AI_Store_Creation_Service.agentic.generation.get_ai_provider_client",
        ...
        self.assertEqual(calls[0]["blueprint"], blueprint)
        self.assertEqual(
            calls[0]["effective_personalization_context"],
            state["effective_personalization_context"],
        )
        self.assertEqual(
            calls[0]["user_store_description"],
            state["normalized_description"],
        )

    def test_generation_prompt_is_not_based_only_on_original_description(self):
        blueprint = self._build_blueprint()
        context = self._effective_context()
        messages = build_generate_agentic_store_draft_messages(
            tenant_id=101,
            store_id=10,
            user_store_description="متجر صابون",
            available_theme_templates=["Earth", "Minimal"],
            blueprint=blueprint,
            effective_personalization_context=context,
        )
        ...
        self.assertIn("normalized_original_description:", contents)
        self.assertIn("store_blueprint:", contents)
        self.assertIn("effective_personalization_context:", contents)
        self.assertIn("locked personalization constraints", messages[0]["content"].lower())
        self.assertIn("categories and products must match", messages[0]["content"].lower())
        self.assertIn("price_positioning", messages[0]["content"])
        self.assertIn("visual_preferences", messages[0]["content"])

    def test_specialized_store_rejects_unrelated_categories_and_products(self):
        blueprint = self._build_blueprint()
        payload = self._valid_arabic_soap_payload()
        payload["categories"] = [{"name": "إلكترونيات"}, {"name": "هواتف"}]
        payload["products"] = [
            {
                "name": "هاتف ذكي",
                "description": "هاتف إلكتروني حديث للاستخدام اليومي.",
                "price": 1000000,
                "sku": "PHONE-01",
                "category_name": "هواتف",
                "stock_quantity": 2,
        ...
        with self.assertRaises(generation.AIGenerationConstraintError):
            generation.validate_personalization_constrained_draft(
                payload,
                blueprint=blueprint,
                effective_personalization_context=self._effective_context(),
            )

    def test_confirmed_language_and_currency_cannot_change(self):
        blueprint = self._build_blueprint()
        payload = self._valid_arabic_soap_payload()
        generation.validate_personalization_constrained_draft(
            payload,
        ...
        payload["store_settings"]["language"] = "en"
        payload["store_settings"]["currency"] = "USD"
        with self.assertRaises(generation.AIGenerationConstraintError):
            generation.validate_personalization_constrained_draft(
                payload,
                blueprint=blueprint,
    ...
    def test_other_custom_answer_is_preserved_as_locked_blueprint_constraint(self):
        state = self._blueprint_state()
        state["clarification_facts"]["visual_preferences"] = (
            "Muted clay, sand, and olive colors with no bright blue"
        )
        state["effective_personalization_context"] = merge_personalization_facts(
            state["description_personalization_facts"],
            state["clarification_facts"],
        )

        blueprint = self._build_blueprint(state)

        self.assertEqual(
            blueprint["visual_preferences"],
            "Muted clay, sand, and olive colors with no bright blue",
        )
        self.assertEqual(
            blueprint["locked_constraints"]["visual_preferences"],
            "Muted clay, sand, and olive colors with no bright blue",
        )

    def test_no_graph_node_applies_database_changes(self):
        node_sources = "\n".join(
            inspect.getsource(node)
            for node in (
                understand_node,
        ...
        for forbidden in (
            ...
            with self.subTest(forbidden=forbidden):
                self.assertNotIn(forbidden, node_sources)

    def test_no_agentic_path_produces_applied_status(self):
        source = "\n".join(
            inspect.getsource(node)
            for node in (
                understand_node,
        ...
        self.assertNotIn("WORKFLOW_STATUS_APPLIED", source)
        with self.assertRaises(ValueError):
            validate_agentic_terminal_state(
                {
                    "store_id": 10,
                    "tenant_id": 101,
                    "user_id": 7,
                    "status": WORKFLOW_STATUS_APPLIED,
                    "current_step": "human_review",
                    "route_decision": "human_review",
                    "mode": "draft_ready",
                    "clarification_round_count": 0,
                    ...
                    "clarification_questions": [],
                    "validation_errors": [],
                    "draft_payload": self._valid_arabic_soap_payload(),
                }
            )
